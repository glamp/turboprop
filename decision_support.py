...
TRADE_OFF_PATTERNS = [
    {
        ...
        'threshold': 0.3,  # Minimum difference to be considered a trade-off
...
class DecisionSupport:
    ...
    def generate_selection_guidance(
        ...
        try:
            ...
            recommended_tool, confidence = self._apply_recommendation_strategy(
                comparison_result, strategy
            )
            
            # Generate rationale
            key_factors = self._identify_key_decision_factors(comparison_result, recommended_tool)
            why_recommended = self._generate_recommendation_reasons(
                comparison_result, recommended_tool, strategy
            )
            when_to_reconsider = self._generate_reconsideration_scenarios(
                comparison_result, recommended_tool
            )
            
            # Find alternatives
            close_alternatives = self._find_close_alternatives(
                comparison_result, recommended_tool
            )
            fallback_options = self._find_fallback_options(
                comparison_result, recommended_tool
            )
        ...
        for tool_id in comparison_result.compared_tools:
            if tool_id in comparison_result.comparison_matrix:
                ...
                priority_score = statistics.mean(
                    metrics.get(metric, 0.5) for metric in priority_metrics
                )
                
                # Overall score (priority metrics weighted more heavily)
                all_metrics_score = statistics.mean(metrics.values()) if metrics else 0.5
                weighted_score = (priority_score * 0.7) + (all_metrics_score * 0.3)
                
                tool_scores[tool_id] = weighted_score
        
        # Find best tool
        if not tool_scores:
            return comparison_result.compared_tools[0] if comparison_result.compared_tools else "unknown", 0.5
            
        recommended_tool = max(tool_scores.keys(), key=lambda t: tool_scores[t])
        confidence = min(tool_scores[recommended_tool] * 1.2, 1.0)  # Boost confidence slightly
    ...
    def _identify_key_decision_factors(
        ...
        for metric, score in tool_metrics.items():
            if score >= 0.7:  # Tool excels in this metric
    ...
    def _generate_recommendation_reasons(
        ...
        for metric in rule['priority_metrics']:
            score = tool_metrics.get(metric, 0.5)
            if score >= 0.7:
        ...
        comparative_advantage = self._find_comparative_advantages(
            comparison_result, recommended_tool
        )
    ...
    def _find_close_alternatives(
        ...
        recommended_score = statistics.mean(recommended_metrics.values()) if recommended_metrics else 0.5
        ...
        for tool_id in comparison_result.compared_tools:
            if tool_id != recommended_tool and tool_id in comparison_result.comparison_matrix:
                ...
                tool_score = statistics.mean(tool_metrics.values()) if tool_metrics else 0.5
    ...
    def _find_fallback_options(
        ...
        fallbacks = [
            tool for tool in comparison_result.compared_tools 
    ...
    def _analyze_trade_off_pattern(
        ...
        for tool in tools:
            ...
            if tool_id in metrics:
                ...
                score_a = tool_metrics.get(metric_a, 0.5)
                score_b = tool_metrics.get(metric_b, 0.5)
            ...
            if score >= 0.7:
                ...
                for tool_id, tool_metrics in comparison_result.comparison_matrix.items():
                    if tool_id != recommended_tool:
                        other_scores.append(tool_metrics.get(metric, 0.5))
    ...
    def _create_fallback_guidance(self, comparison_result: 'ToolComparisonResult') -> SelectionGuidance:
        ...
        return SelectionGuidance(
            ...
            close_alternatives=comparison_result.compared_tools[1:2] if len(comparison_result.compared_tools) > 1 else [],
