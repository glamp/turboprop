...
class CatalogValidator:
    ...
    def __init__(self, db_manager: Optional[DatabaseManager] = None):
        ...
        self.quality_thresholds = {"excellent": 0.9, "good": 0.7, "acceptable": 0.5, "poor": 0.0}
    ...
    def generate_quality_metrics(self) -> Dict[str, Any]:
        ...
        try:
            with self.db_manager.get_connection() as conn:
                ...
                embedding_count = conn.execute("SELECT COUNT(*) FROM mcp_tools WHERE embedding IS NOT NULL").fetchone()[
                    0
                ]
                metrics["tools_with_embeddings"] = embedding_count

                # Parameter coverage
                tools_with_params = conn.execute("SELECT COUNT(DISTINCT tool_id) FROM tool_parameters").fetchone()[0]
                metrics["parameter_coverage"] = (
                    tools_with_params / metrics["total_tools"] if metrics["total_tools"] > 0 else 0.0
                )

                # Example coverage
                tools_with_examples = conn.execute("SELECT COUNT(DISTINCT tool_id) FROM tool_examples").fetchone()[0]
                metrics["example_coverage"] = (
                    tools_with_examples / metrics["total_tools"] if metrics["total_tools"] > 0 else 0.0
                )
                ...
                metrics["relationship_coverage"] = (
                    total_relationships / max_possible_relationships if max_possible_relationships > 0 else 0.0
                )
                ...
                metrics["metadata_completeness"] = (
                    tools_with_descriptions / metrics["total_tools"] if metrics["total_tools"] > 0 else 0.0
                )
    ...
    def _validate_examples(self, examples: List[ToolExample]) -> Dict[str, Any]:
        ...
        for example in examples:
            if (
                hasattr(example, "use_case")
                and example.use_case
                and hasattr(example, "example_call")
                and example.example_call
            ):
                complete_examples += 1
    ...
    def _check_table_existence(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "errors": [f"Error checking table existence: {e}"], "warnings": []}
    ...
    def _check_foreign_key_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "errors": [f"Error checking foreign key integrity: {e}"], "warnings": []}
    ...
    def _check_embedding_dimensions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "errors": [f"Error checking embedding dimensions: {e}"], "warnings": []}
    ...
    def _check_data_completeness(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "errors": [f"Error checking data completeness: {e}"], "warnings": []}
    ...
    def _check_relationship_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "errors": [f"Error checking relationship consistency: {e}"], "warnings": []}
