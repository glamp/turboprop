...
from indexing_operations import embed_and_store_single, scan_repo
        ...
        with self.lock:
            ...
            try:
                self.db_manager.execute_with_retry(
                    "DELETE FROM code_files WHERE path = ?",
                    (str(file_path),)
                )
...
def watch_mode(repo_path: str, max_mb: float, debounce_sec: float):
    ...
    try:
        print(f"👀 Watching {repo_path} for changes (debounce: {debounce_sec}s)", file=sys.stderr)
