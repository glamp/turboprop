    ...
    def queue_write(self, file_path: Path, data: Dict[str, Any], priority: str = "normal") -> bool:
        ...
        try:
            with self._batch_lock:
                self._pending_writes[str(file_path)] = {
                    "data": data,
                    "priority": priority,
                    "queued_at": time.time()
                }
        ...
        try:
            with self._batch_lock:
                ...
                sorted_writes = sorted(
                    ...
                    key=lambda x: (x[1]["priority"] == "high", -x[1]["queued_at"])
                )
        ...
        with self._batch_lock:
            ...
            return {
                ...
                "oldest_queued_write": min(
                    (current_time - write_info["queued_at"] for write_info in self._pending_writes.values()),
                    default=0
                )
            }
