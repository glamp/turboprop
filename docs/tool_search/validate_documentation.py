...
class DocumentationValidator:
    ...
    def _check_function_signatures(self):
        ...
        expected_signatures = {
            'search_mcp_tools': [
                'query', 'category', 'tool_type', 'max_results', 
                'include_examples', 'search_mode'
            ],
            ...
            'compare_mcp_tools': [
                'tool_ids', 'comparison_criteria', 'include_decision_guidance',
                'comparison_context', 'detail_level'
            ]
        }
                ...
                for call in calls:
                    ...
                    if unexpected:
                        inconsistencies.append(
                            f"{py_file.name}: {func_name} uses unexpected parameters: {unexpected}"
                        )
    ...
    def _check_terminology_consistency(self):
        ...
        canonical_terms = {
            'MCP Tool Search System': [
                'tool search system', 'Tool Search System', 'mcp tool search'
            ],
                ...
                for variation in variations:
                    if variation in content:
                        terminology_issues.append(
                            f"{md_file.name}: Use '{canonical}' instead of '{variation}'"
                        )
        ...
        for func_name, examples in code_examples.items():
            if len(examples) > 1:
                ...
                if len(set(tuple(p[1]) for p in param_patterns)) > 1:
                    inconsistencies.append(
                        f"{func_name} examples have inconsistent parameter usage"
                    )
        ...
        if reference_file.exists():
            ...
            if missing_docs:
                self.warnings.extend([
                    f"Tool missing from reference docs: {tool}" 
    ...
    def _check_required_sections(self):
        ...
        required_sections = {
            'user_guide.md': [
                'Overview', 'Key Features', 'Search Strategies', 
                'Best Practices', 'Troubleshooting'
            ],
            'api_reference.md': [
                'Core MCP Tools', 'search_mcp_tools', 'recommend_tools_for_task',
                'Error Handling', 'Performance Considerations'
            ],
            'architecture.md': [
                'Overview', 'Core Components', 'Data Flow', 'Performance'
            ]
        }
