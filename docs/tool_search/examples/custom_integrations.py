    ...
    def _apply_filters(self, tools: List[Dict], filters: Dict[str, Any]) -> List[Dict]:
        ...
        if 'complexity_max' in filters:
            ...
            filtered_tools = [
                ...
                if tool.get('complexity_score', 0.5) <= max_complexity
            ]
...
async def run_custom_integration_examples():
    ...
    try:
        domain_results = await domain_algorithm.search(
    ...
    category_manager.register_category(
        ...
        'Tools for processing and transforming data',
        {
            'keywords': {
                'required': ['data'],
                'optional': ['process', 'transform', 'parse', 'convert']
            },
    ...
    scoring_system.set_user_preferences('user123', {
        'complexity': 'balanced',
        'reliability_weight': 1.5,
        'categories': ['data_processing', 'web_development']
    })
    
    try:
        scored_tools = await scoring_system.score_tools(
            mock_tools,
    ...
    external_integration.register_external_catalog('github_tools', {
        'type': 'api',
        'endpoint': 'https://api.github.com/tools',
        'auth': {'token': 'mock_token'}
    })
    
    external_integration.register_external_catalog('internal_db', {
        'type': 'database',
    ...
    try:
        ...
        federated_results = await external_integration.search_federated(
