...
class ConstructSearchResult:
    ...
    This class extends the concept of CodeSearchResult specifically for constructs,
    including construct-specific information like signatures, parent relationships,
    and construct type.
    """
    construct_id: str
    file_path: str
...
class ConstructSearchOperations:
    ...
    def search_constructs(
        ...
        try:
            ...
            construct_results = []
            for row in results:
                construct_results.append(ConstructSearchResult(
                    construct_id=row[0],
                    file_path=row[1],
            ...
            logger.info(f"Found {len(construct_results)} construct matches for query: {query}")
            return construct_results
            
        except Exception as e:
            logger.error(f"Error searching constructs for query '{query}': {e}")
            return []
        ...
        try:
            ...
            with self.db_manager.get_connection() as conn:
                ...
                if not construct_info:
                    logger.warning(f"Construct not found: {construct_id}")
                    return []
                
                file_id, parent_construct_id, construct_type, name = construct_info
                ...
                related_constructs = []
                for row in results:
                    related_constructs.append(ConstructSearchResult(
                        construct_id=row[0],
                        file_path=row[1],
                        construct_type=row[2],
                        name=row[3],
                ...
                logger.info(f"Found {len(related_constructs)} related constructs for {construct_id}")
                return related_constructs
                
        except Exception as e:
            logger.error(f"Error finding related constructs for {construct_id}: {e}")
            return []
    ...
    def get_construct_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error getting construct statistics: {e}")
            return {'error': str(e)}
