...
class ToolMetadataExtractor:
    ...
    CATEGORY_PATTERNS = {
        ...
        'notebook': [
            r'\b(notebook|jupyter|ipynb|cell)\b',
            r'\b(python|interactive)\b'
        ],
        'workflow': [
            r'\b(workflow|plan|mode|exit|transition)\b',
            r'\b(process|step|stage)\b'
        ]
    }
    ...
    def extract_tool_metadata(self, tool_def: Dict[str, Any]) -> MCPToolMetadata:
        ...
        logger.debug("Extracted metadata for %s: category=%s, %d parameters", 
                    name, category, len(parameters))
