...
from alternative_detector import AlternativeAnalysis, AlternativeDetector
...
class ToolComparisonEngine:
    ...
    def compare_tools(
        ...
        try:
            ...
            preliminary_result = ToolComparisonResult(
                ...
                comparison_criteria=comparison_criteria or list(comparison_matrix.get(tool_ids[0], {}).keys()),
                context_factors=self._extract_context_factors(context),
                confidence_scores=confidence_scores
            )
            
            decision_guidance = self.decision_support.generate_selection_guidance(
                preliminary_result, context
            )
    ...
    def compare_for_task(
        ...
        try:
            ...
            task_fit_scores = self._calculate_task_fit_scores(
                tools_data, task_description, comparison_matrix
            )
            
            # Step 5: Generate task-specific ranking
            task_ranking = self._calculate_task_specific_ranking(
                comparison_matrix, task_fit_scores
            )
    ...
    def _create_mock_tool_data(self, tool_id: str) -> ToolSearchResult:
        ...
        param_data = {
            ...
            'bash': [
                ParameterAnalysis(name="command", type="string", required=True, description="Command to execute")
            ]
        }
    ...
    def _filter_by_criteria(
        ...
        for tool_id, metrics in comparison_matrix.items():
            filtered_metrics = {
                criterion: score for criterion, score in metrics.items()
    ...
    def _calculate_category_rankings(
        self,
        tools_data: List[ToolSearchResult],
        comparison_matrix: Dict[str, Dict[str, float]]
    ) -> Dict[str, List[str]]:
    ...
    def _identify_key_differentiators(
        ...
        sorted_metrics = sorted(
            metric_variances.keys(),
            key=lambda m: metric_variances[m],
            reverse=True
        )
    ...
    def _calculate_task_specific_ranking(
        self,
        comparison_matrix: Dict[str, Dict[str, float]],
        task_fit_scores: Dict[str, float]
    ) -> List[str]:
    ...
    def _perform_detailed_analysis(
        ...
        tool_a_advantages = [
            f"{tool_a} offers simpler interface",
            f"{tool_a} has better performance for its use case"
        ]
        ...
        use_case_scenarios = {
            ...
            tool_b: [
                f"Use {tool_b} for complex, multi-step operations",
                f"{tool_b} excels in modification scenarios"
            ]
        }
