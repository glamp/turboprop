...
class ToolStorageOperations:
    ...
    def store_tool_batch(self, tools: List[MCPToolMetadata]) -> StorageResult:
        ...
        if not tools:
            ...
            return StorageResult(success=True, tools_stored=0, tools_failed=0, errors=[], execution_time=0.0)
        ...
        try:
            ...
            for tool in tools:
                try:
                    ...
                    parameter_ops = self._create_parameter_operations(tool_id, getattr(tool, "parameters", []))
                    operations.extend(parameter_ops)

                    # Add example operations
                    example_ops = self._create_example_operations(tool_id, getattr(tool, "examples", []))
                    operations.extend(example_ops)
            ...
            logger.info(
                "Batch storage completed: %d stored, %d failed in %.2fs", tools_stored, tools_failed, execution_time
            )
    ...
    def _extract_additional_metadata(self, tool: MCPToolMetadata) -> Dict[str, Any]:
        ...
        if hasattr(tool, "usage_patterns"):
            metadata["usage_patterns"] = [pattern.to_dict() for pattern in tool.usage_patterns]
