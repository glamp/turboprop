...
COMPARISON_METRICS = {
    'functionality': {
        ...
        'normalization_max': 20.0  # Maximum expected parameter count for normalization
    },
    'usability': {
        'weight': 0.20,
...
class ComparisonMetrics:
    ...
    def calculate_all_metrics(
        ...
        for tool in tools:
            try:
                ...
                tool_metrics['functionality'] = self.calculate_functionality_score(tool)
                tool_metrics['usability'] = self.calculate_usability_score(tool)
                tool_metrics['reliability'] = self.calculate_reliability_score(tool)
                tool_metrics['performance'] = self.calculate_performance_score(tool)
                tool_metrics['compatibility'] = self.calculate_compatibility_score(tool, context)
                tool_metrics['documentation'] = self.calculate_documentation_score(tool)
                
                # Ensure all scores are normalized
                tool_metrics = {k: self._normalize_score(v) for k, v in tool_metrics.items()}
                
                results[tool_id] = tool_metrics
                ...
                results[str(tool.tool_id)] = {
                    metric: self.scoring_config['default_score'] 
    ...
    def calculate_functionality_score(self, tool: ToolSearchResult) -> float:
        ...
        try:
            ...
            param_count_score = min(tool.parameter_count / 10.0, 1.0) if tool.parameter_count else 0.2
            ...
            # Combine factors
            raw_score = (
                param_count_score * 0.4 +
                complexity_score * 0.3 +
                description_richness * 0.2 +
                category_bonus * 0.1
            )
    ...
    def calculate_usability_score(self, tool: ToolSearchResult) -> float:
        ...
        try:
            ...
            required_param_score = 1.0 - (req_ratio * 0.7)  # Penalty for many required params
    ...
    def calculate_compatibility_score(self, tool: ToolSearchResult, context: Optional[TaskContext] = None) -> float:
        ...
        try:
            ...
            # Combine factors
            raw_score = (
                io_compatibility * 0.3 +
                param_compatibility * 0.3 +
                ecosystem_fit * 0.25 +
                error_compat * 0.15
            )
    ...
    def calculate_documentation_score(self, tool: ToolSearchResult) -> float:
        ...
        try:
            ...
            raw_score = (
                desc_quality * 0.5 +
                param_doc_quality * 0.3 +
                example_indicators * 0.2
            )
    ...
    def _calculate_description_richness(self, description: str) -> float:
        ...
        technical_terms = len(re.findall(r'\b(file|data|process|execute|create|update|delete|search|query|analyze)\b', description.lower()))
    ...
    def _calculate_description_clarity(self, description: str) -> float:
        ...
        jargon_penalty = min(jargon_indicators / 10.0, 0.4)
    ...
    def _calculate_parameter_documentation_quality(self, tool: ToolSearchResult) -> float:
        ...
        for param in tool.parameters:
            if param.description:
                ...
                specificity = 1.0 if any(word in param.description.lower() for word in ['example', 'e.g.', 'such as', 'format']) else 0.5
                
                quality_sum += (length_factor * 0.6 + specificity * 0.4)
            
        avg_quality = quality_sum / len(tool.parameters) if tool.parameters else 0.0
        
        return (documentation_ratio * 0.7 + avg_quality * 0.3)
    ...
    def _detect_validation_features(self, tool: ToolSearchResult) -> float:
        ...
        for param in tool.parameters:
            if param.description and any(word in param.description.lower() for word in ['valid', 'format', 'pattern', 'required', 'optional']):
    ...
    def _assess_tool_maturity(self, tool: ToolSearchResult) -> float:
        ...
        if tool.description and any(word in tool.description.lower() for word in ['stable', 'reliable', 'tested', 'proven']):
            maturity_score += 0.2
    ...
    def _assess_parameter_safety(self, tool: ToolSearchResult) -> float:
        ...
        if any(word in tool.description.lower() for word in ['delete', 'remove', 'overwrite', 'replace']):
            ...
            has_safety_params = any(word in param.name.lower() for param in tool.parameters 
    ...
    def _assess_io_compatibility(self, tool: ToolSearchResult) -> float:
        ...
        if tool.parameters:
            has_input_param = any('input' in param.name.lower() or 'file' in param.name.lower() 
                                 for param in tool.parameters)
            has_output_param = any('output' in param.name.lower() or 'result' in param.name.lower() 
                                  for param in tool.parameters)
    ...
    def _assess_parameter_type_compatibility(self, tool: ToolSearchResult) -> float:
        ...
        compatible_params = sum(1 for p in tool.parameters 
                               if p.type and p.type.lower() in standard_types)
    ...
    def _calculate_description_quality(self, description: str) -> float:
        ...
        # Technical content
        technical_terms = len(re.findall(r'\b(parameter|function|method|class|object|data|file|process)\b', description.lower()))
    ...
    def _normalize_score(self, score: float) -> float:
        ...
        return max(
            self.scoring_config['min_score'], 
            min(score, self.scoring_config['max_score'])
        )