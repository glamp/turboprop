...
"""
    ...
    def _format_detailed_comparison_table(self, comparison_result: 'ToolComparisonResult') -> str:
        ...
        # Header
        header = self._format_header("DETAILED TOOL COMPARISON")
        sections.append(header)
...
"""
        ...
        if comparison_result.comparison_matrix:
            matrix_section = f"""
{self._format_subheader("Comparison Matrix")}
{self._format_metrics_table(comparison_result.comparison_matrix)}
"""
...
"""
        ...
        # Key differentiators
        if comparison_result.key_differentiators:
            diff_section = f"""
{self._format_subheader("Key Differentiators")}
{self._format_bullet_list(comparison_result.key_differentiators)}
"""
            sections.append(diff_section.strip())
        
        # Decision guidance
        if comparison_result.decision_guidance:
            guidance_section = f"""
{self._format_subheader("Decision Guidance")}
Recommended: {self._colorize(comparison_result.decision_guidance.recommended_tool, 'success')}
Confidence: {comparison_result.decision_guidance.confidence:.1%}
"""
    ...
    def _format_summary_comparison_table(self, comparison_result: 'ToolComparisonResult') -> str:
        ...
        # Header
        header = self._format_header("TOOL COMPARISON SUMMARY")
        sections.append(header)
        ...
        if filtered_matrix:
            matrix_section = f"""
{self._format_subheader("Key Metrics Comparison")}
{self._format_metrics_table(filtered_matrix)}
"""
            sections.append(matrix_section.strip())
        ...
        if comparison_result.decision_guidance:
            quick_rec = f"""
{self._format_subheader("Quick Recommendation")}
→ {self._colorize(comparison_result.decision_guidance.recommended_tool, 'success')} ({comparison_result.decision_guidance.confidence:.0%} confidence)
"""
            sections.append(quick_rec.strip())
    ...
    def _format_metrics_table(self, comparison_matrix: Dict[str, Dict[str, float]]) -> str:
        ...
        metric_width = max(8, max(len(metric.title()) for metric in metrics) if metrics else 8)  # Dynamic width based on longest metric name
    ...
    def _format_single_trade_off(self, trade_off: TradeOffAnalysis, index: int) -> str:
        ...
        importance_color = {
            ...
            'important': 'warning',
            'minor': 'info'
        }.get(trade_off.decision_importance, 'info')
        
        header = f"""
{self._format_subheader(f"Trade-off #{index}: {trade_off.trade_off_name.replace('_', ' ').title()}")}
