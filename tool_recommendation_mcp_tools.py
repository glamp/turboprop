...
from task_analysis_mcp_responses import (
    AlternativeToolMeta,
    AlternativesResponse,
    TaskAnalysisResponse,
    TaskRecommendationResponse,
    ToolRecommendationMeta,
...
def initialize_recommendation_tools(
    recommendation_engine: ToolRecommendationEngine,
...
def recommend_tools_for_task(
    ...
    try:
        ...
        if not task_description or not isinstance(task_description, str):
            return json.dumps(create_error_response("recommend_tools_for_task", "Task description must be a non-empty string"))
        
        task_description = task_description.strip()
        if not task_description:
            return json.dumps(create_error_response("recommend_tools_for_task", "Task description cannot be empty"))
        
        if len(task_description) > TASK_DESCRIPTION_MAX_LENGTH:
            return json.dumps(create_error_response("recommend_tools_for_task", f"Task description too long (max {TASK_DESCRIPTION_MAX_LENGTH} characters)"))
        ...
        if not _recommendation_engine or not _task_analyzer:
            return json.dumps(create_error_response("recommend_tools_for_task", "Recommendation engine not initialized"))
        ...
        for rec in engine_response.recommendations:
            mcp_rec = ToolRecommendationMeta(
        ...
        response = TaskRecommendationResponse(
            ...
            task_analysis=task_analysis.to_dict() if task_analysis else None,
            context_factors=task_context.to_dict() if task_context else None,
            processing_time=time.time() - start_time,
            complexity_assessment=task_analysis.complexity_level if task_analysis else None,
            skill_level_guidance=task_analysis.skill_level_required if task_analysis else None
        )
        ...
        if _suggestion_generator and task_analysis:
            suggestions = _suggestion_generator.generate_task_description_suggestions(
        ...
        logger.error(f"Error in recommend_tools_for_task: {str(e)}", exc_info=True)
        return json.dumps(create_error_response("recommend_tools_for_task", str(e), task_description))
...
def analyze_task_requirements(
    ...
    try:
        # Validate inputs
        if not task_description or not isinstance(task_description, str):
            return json.dumps(create_error_response("analyze_task_requirements", "Task description must be a non-empty string"))
        ...
        if len(task_description) > TASK_DESCRIPTION_MAX_LENGTH:
            return json.dumps(create_error_response("analyze_task_requirements", f"Task description too long (max {TASK_DESCRIPTION_MAX_LENGTH} characters)"))
        ...
        response = TaskAnalysisResponse(
            ...
            estimated_complexity=analysis.complexity_level if analysis else "moderate",
...
def suggest_tool_alternatives(
    ...
    try:
        ...
        if not primary_tool or not isinstance(primary_tool, str):
            return json.dumps(create_error_response("suggest_tool_alternatives", "Primary tool must be a non-empty string"))
        ...
        if not _recommendation_engine:
            return json.dumps(create_error_response("suggest_tool_alternatives", "Recommendation engine not initialized"))
        ...
        if include_comparisons and mcp_alternatives and _comparison_formatter:
            comparisons = _comparison_formatter.generate_alternative_comparisons(
                primary_tool, mcp_alternatives
            )
...
def recommend_tool_sequence(
    ...
    try:
        ...
        if not workflow_description or not isinstance(workflow_description, str):
            return json.dumps(create_error_response("recommend_tool_sequence", "Workflow description must be a non-empty string"))
        ...
        if len(workflow_description) > TASK_DESCRIPTION_MAX_LENGTH * 2:
            return json.dumps(create_error_response("recommend_tool_sequence", f"Workflow description too long (max {TASK_DESCRIPTION_MAX_LENGTH * 2} characters)"))
        ...
        for i, seq in enumerate(engine_response.sequences):
            ...
            steps = []
            for j, tool_id in enumerate(getattr(seq, 'tool_chain', [seq.recommended_tool_id if hasattr(seq, 'recommended_tool_id') else 'unknown'])):
            ...
            sequence_meta = ToolSequenceMeta(
...
def tool_exists(tool_name: str) -> bool:
    ...
    common_tools = [
        'bash', 'read', 'write', 'edit', 'glob', 'grep', 'search_code', 
        'index_repository', 'list_indexed_files', 'watch_repository',
        'search_mcp_tools', 'get_tool_details'
    ]
