...
from task_analysis_mcp_responses import ToolRecommendationMeta
...
class MCPExplanationFormatter:
    ...
    def __init__(self):
        ...
        self.reason_categories = {
            'functionality': ['provides required functionality', 'supports needed operations', 'handles specified tasks'],
    ...
    def generate_recommendation_explanations(
        self, 
        recommendations: List[ToolRecommendationMeta], 
        ...
        for i, rec in enumerate(recommendations):
            try:
                ...
                logger.warning(f"Failed to generate explanation for {rec.tool_name}: {e}")
                # Fallback explanation
                explanations.append(f"#{i+1} {rec.tool_name}: Recommended for your task with {rec.confidence_score:.1%} confidence.")
        
        return explanations
    
    def _format_single_recommendation_explanation(
        self, 
        rec: ToolRecommendationMeta, 
        rank: int, 
        task_analysis: Optional[TaskAnalysis]
    ) -> str:
        """
        ...
        """
        parts = [f"#{rank} {rec.tool_name}:"]
        
        # Add primary reasons
        if rec.recommendation_reasons:
            reasons_text = self._format_reasons(rec.recommendation_reasons[:2])
            parts.append(f"Recommended because {reasons_text}.")
        
        # Add task alignment info
        alignment_text = self._format_task_alignment(rec.task_alignment)
        if alignment_text:
            parts.append(alignment_text)
        
        # Add complexity fit
        if rec.complexity_fit != "unknown":
            complexity_text = self._format_complexity_fit(rec.complexity_fit, task_analysis)
            if complexity_text:
                parts.append(complexity_text)
        
        # Add usage guidance
        if rec.usage_guidance:
            guidance_text = f"Best used when {rec.usage_guidance[0]}"
            parts.append(guidance_text)
        
        # Add confidence indicator
        confidence_text = self._format_confidence_indicator(rec.confidence_score)
        if confidence_text:
...
class AlternativeComparisonFormatter:
    ...
    def generate_alternative_comparisons(
        self, 
        primary_tool: str, 
    ...
    def _format_single_alternative_comparison(
        self, 
        primary_tool: str, 
        alt_name: str, 
        alternative: Any
...
def create_confidence_summary(recommendations: List[ToolRecommendationMeta]) -> str:
    """
