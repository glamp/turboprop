...
from search_operations import search_hybrid, search_with_construct_focus
    ...
    def test_hybrid_search_end_to_end(self, sample_repo, fully_mock_db_manager, mock_embedder):
        ...
        with patch('hybrid_search.search_index_enhanced') as mock_search:
            ...
            for mode in [SearchMode.AUTO, SearchMode.HYBRID, SearchMode.SEMANTIC_ONLY, SearchMode.TEXT_ONLY]:
                ...
                assert len(results) > 0
                assert all(hasattr(r, 'fusion_score') or hasattr(r, 'semantic_score') or hasattr(r, 'text_score') for r in results)
