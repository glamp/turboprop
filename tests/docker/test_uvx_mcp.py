...
class UVXMCPTester:
    ...
    def run_command(
        self, cmd: List[str], timeout: int = 300, capture_output: bool = True
    ...
    def test_mcp_server_startup(self) -> bool:
        ...
        cmd = (
            ["uvx", "turboprop", "mcp", "--repository", ".", "--auto-index"]
            if hasattr(self, "_turboprop_installed") and self._turboprop_installed
            else ["uvx", "turboprop@latest", "mcp", "--repository", ".", "--auto-index"]
        )
        ...
        try:
            ...
            while time.time() - start_time < startup_timeout:
                ...
                if (
                    "Indexing complete" in recent_stderr
                    or "ready for semantic search" in recent_stderr
                    or "MCP Server Starting" in recent_stderr
                ):
            ...
            success_indicators = ["Turboprop MCP Server Starting", "Repository:", "Max file size:", "Auto-index: Yes"]
    ...
    def test_repository_structure(self) -> bool:
        ...
        expected_files = ["package.json", "src/pages/index.tsx", "src/widgets/Player.tsx", "README.md"]
