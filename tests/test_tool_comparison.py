...
class TestToolComparisonEngine:
    ...
    def test_get_detailed_comparison(self):
        ...
        mock_detailed = DetailedComparison(
            ...
            switching_guidance=["Use read for viewing only", "Use edit when modification needed"],
            confidence=0.9,
        )
...
class TestAlternativeDetector:
    ...
    def setup_method(self):
        ...
        self.detector = AlternativeDetector(
            tool_search_engine=self.mock_search_engine, similarity_analyzer=self.mock_similarity_analyzer
        )
    ...
    def test_find_alternatives_basic(self):
        ...
        self.mock_similarity_analyzer.calculate_functional_similarity.side_effect = (
            lambda ref, alt: mock_similarity_scores.get(alt, 0.5)
        )
    ...
    def test_find_alternatives_with_threshold(self):
        ...
        self.mock_search_engine.search_by_functionality.return_value.results = mock_search_results
        self.mock_similarity_analyzer.calculate_functional_similarity.side_effect = lambda ref, alt: {
            "regex_search": 0.85,
            "find": 0.6,
        }.get(alt, 0.5)
...
class TestComparisonMetrics:
    ...
    def test_calculate_functionality_score(self):
        ...
        complex_tool = ToolSearchResult(
            ...
            parameters=[
                ParameterAnalysis(name=f"param_{i}", type="string", required=i < 5, description=f"Parameter {i}")
                for i in range(10)
            ],
        )
    ...
    def test_calculate_usability_score(self):
        ...
        complex_tool = ToolSearchResult(
            ...
            parameters=[
                ParameterAnalysis(name=f"param_{i}", type="object", required=i < 6, description="") for i in range(8)
            ],
        )
