...
from tool_recommendation_engine import ToolRecommendationEngine, RecommendationRequest
...
class TestRecommendationSystemIntegration:
    ...
    def complete_system(self):
        ...
        mock_tool_search.search_tools.return_value = [
            Mock(tool_id="csv_reader", name="CSV Reader", description="Read CSV files", 
                 score=0.9, metadata={"capabilities": ["read", "csv"], "complexity": "simple"}),
            Mock(tool_id="data_processor", name="Data Processor", description="Process data",
                 score=0.8, metadata={"capabilities": ["process", "data"], "complexity": "moderate"}),
        ]
    ...
    def test_complete_recommendation_flow_basic(self, complete_system):
        ...
        assert "read" in response.task_analysis.required_capabilities or "csv" in response.task_analysis.required_capabilities
