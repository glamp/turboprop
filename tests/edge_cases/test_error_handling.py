        ...
        def mock_generate_embeddings(texts):
            ...
            for text in texts:
                if "error" in text.lower():
                    raise Exception(f"Failed to embed text with 'error'")
                else:
...
class TestResourceExhaustionScenarios:
    ...
    def test_concurrent_access_stress(self, sample_repo, mock_embedder):
        ...
        try:
            ...
            def reindex_worker():
                try:
                    total, processed, elapsed = reindex_all(sample_repo, int(10.0 * 1024 * 1024), db_manager, mock_embedder)
                    return processed
                except Exception as e:
                    return str(e)
