...
from mcp_tool_discovery import MCPTool, MCPToolDiscovery, ParameterInfo, ToolExample as MCPToolExample
...
class TestToolEmbeddingPipeline(unittest.TestCase):
    ...
    def setUp(self):
        ...
        batch_embeddings = np.array([
            np.random.normal(0, 0.3, 384).astype(np.float32),
            np.random.normal(0, 0.3, 384).astype(np.float32)
        ])
    ...
    def test_generate_tool_embeddings_processes_single_tool(self):
        ...
        metadata = MCPToolMetadata(
            name="Test Tool",
            description="A test tool for embedding generation",
            category="testing"
        )
...
class TestToolRelationshipDetector(unittest.TestCase):
    ...
    def test_detect_alternatives_finds_similar_tools(self):
        ...
        alternative_tool = MCPToolMetadata(
            ...
            parameters=[
                ParameterAnalysis(
                    ...
                    description="Name of file to display"
                )
            ]
        )
        ...
        alternative_relationship = next(
            (r for r in relationships 
             if r.relationship_type == "alternative" 
             and {r.tool_a_name, r.tool_b_name} == {"Read File", "Cat File"}),
            None
        )
    ...
    def test_detect_complements_finds_workflow_tools(self):
        ...
        complement_relationship = next(
            (r for r in relationships 
             if r.relationship_type == "complement"
             and {r.tool_a_name, r.tool_b_name} == {"Read File", "Write File"}),
            None
        )
    ...
    def test_detect_prerequisites_identifies_setup_tools(self):
        ...
        write_tool = MCPToolMetadata(
            name="Write File",
            description="Write file to directory (requires directory to exist)",
            category="file_ops"
        )
...
class TestCatalogValidator(unittest.TestCase):
    ...
    def test_validate_tool_metadata_accepts_complete_metadata(self):
        ...
        complete_metadata = MCPToolMetadata(
            ...
            examples=[
                ToolExample(
                    use_case="Basic usage",
                    example_call="complete_tool(param1='value')",
...
class TestToolCatalogManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mock_metadata = MCPToolMetadata(
            name="Mock Tool",
            description="Mock tool for testing with sufficient description length",
            category="testing"
        )
        ...
        mock_metadata.examples = [
            ToolExample(
                use_case="testing",
                example_call="mock_tool({'param': 'value'})",
    ...
    def test_validate_catalog_health_returns_comprehensive_report(self, mock_relationship_detector_class):
        ...
        for i in range(12):  # Create 12 relationships to get >10% coverage
            fake_relationships.append(ToolRelationship(
                tool_a_id=f"tool_{i}", tool_a_name=f"Tool {i}",
                tool_b_id=f"tool_{i+1}", tool_b_name=f"Tool {i+1}",
                relationship_type="alternative" if i % 2 == 0 else "complement",
                strength=0.8, description=f"Relationship {i}", confidence=0.8
            ))
