...
class ToolCatalogManager:
    ...
    def __init__(self, 
        ...
        # Configuration
        self.batch_size = 50  # Process tools in batches
    ...
    def full_catalog_rebuild(self) -> CatalogRebuildResult:
        ...
        try:
            ...
            try:
                ...
                return self._create_failed_rebuild_result(start_time, errors)
            ...
            try:
                ...
                tools_processed = len(all_discovered)
                phase_timings['discovery'] = time.time() - phase_start
                logger.info("Discovery completed: %d tools found in %.2fs", 
                          tools_processed, phase_timings['discovery'])
                
            except Exception as e:
                error_msg = f"Discovery phase failed: {e}"
                errors.append(error_msg)
                logger.error(error_msg)
                return self._create_failed_rebuild_result(start_time, errors, phase_timings)
            ...
            try:
                ...
                phase_timings['analysis'] = time.time() - phase_start
                logger.info("Analysis completed: %d tools analyzed in %.2fs", 
                          len(analyzed_tools), phase_timings['analysis'])
            ...
            try:
                ...
                phase_timings['embedding'] = time.time() - phase_start
                logger.info("Embedding completed: %d embeddings in %.2fs", 
                          embeddings_generated, phase_timings['embedding'])
            ...
            try:
                ...
                phase_timings['relationships'] = time.time() - phase_start
                logger.info("Relationship detection completed: %d relationships in %.2fs", 
                          relationships_created, phase_timings['relationships'])
            ...
            try:
                ...
                if all_relationships:
                    try:
                        self.storage_ops.store_tool_relationships(all_relationships)
                        logger.info("Stored %d relationships", len(all_relationships))
                    except Exception as e:
                        error_msg = f"Failed to store relationships: {e}"
                        errors.append(error_msg)
                        logger.error(error_msg)
                
                phase_timings['storage'] = time.time() - phase_start
                logger.info("Storage completed: %d tools stored in %.2fs", 
                          tools_stored, phase_timings['storage'])
            ...
            try:
                ...
                if not consistency_report.is_consistent:
                    ...
                    logger.warning("Catalog consistency issues found: %d errors", 
                                 len(consistency_report.errors))
                
                phase_timings['validation'] = time.time() - phase_start
                logger.info("Validation completed in %.2fs", phase_timings['validation'])
            ...
            logger.info(
                ...
                success, tools_stored, tools_processed, execution_time
            )
    ...
    def incremental_catalog_update(self) -> CatalogUpdateResult:
        ...
        try:
            ...
            logger.info("Changes detected: %d new, %d updated, %d removed", 
            ...
            logger.info(
                ...
                success, tools_added, tools_updated, execution_time
            )
    ...
    def validate_catalog_health(self) -> CatalogHealthReport:
        ...
        try:
            ...
            try:
                ...
                embedding_coverage = quality_metrics.get('tools_with_embeddings', 0) / max(quality_metrics.get('total_tools', 1), 1)
            ...
            try:
                with self.db_manager.get_connection() as conn:
                    ...
                    checks['relationship_coverage'] = {
                        'status': 'pass' if relationship_coverage > 0.1 else 'warning' if relationship_coverage > 0.05 else 'fail',
                    ...
                    if relationship_coverage < 0.05:
                        recommendations.append(f"Consider running relationship detection - only {relationship_coverage:.2%} coverage")
            ...
            logger.info(
                ...
                is_healthy, overall_health_score, len(critical_issues), validation_time
            )
    ...
    def _create_failed_rebuild_result(self, start_time: float, errors: List[str], 
                                    phase_timings: Optional[Dict[str, float]] = None) -> CatalogRebuildResult:
