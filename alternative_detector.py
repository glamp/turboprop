...
class AlternativeDetector:
    ...
    def analyze_alternative_advantages(
        self,
        primary_tool: str,
        alternative_tool: str
    ) -> AlternativeAdvantageAnalysis:
        """
        ...
        try:
            ...
            switching_cost, migration_considerations = self._analyze_switching_cost(
                primary_tool, alternative_tool
            )
    ...
    def _find_candidate_alternatives(
        ...
        try:
            ...
            search_response = self.tool_search_engine.search_by_functionality(
                query=search_query,
                k=max_candidates
            )
    ...
    def _extract_mock_capabilities(self, tool_id: str) -> List[str]:
        ...
        return capability_map.get(tool_id, ['general_functionality'])
    ...
    def _analyze_alternative(
        ...
        similarity_score = self.similarity_analyzer.calculate_functional_similarity(
            reference_tool, candidate_id
        )
    ...
    def _generate_advantages(
        ...
        for capability in unique_capabilities:
            if capability == 'batch_editing':
                advantages.append("Supports batch operations on multiple files")
            elif capability == 'pattern_matching':
                advantages.append("Advanced pattern matching capabilities")
            elif capability == 'workflow_control':
                advantages.append("Better workflow integration and control")
    ...
    def _generate_disadvantages(
        ...
        for gap in capability_gaps:
            if gap == 'file_writing':
                disadvantages.append("Cannot modify files directly")
            elif gap == 'pattern_matching':
                disadvantages.append("Limited pattern matching capabilities")
            elif gap == 'batch_editing':
                disadvantages.append("No batch operation support")
    ...
    def _generate_preference_scenarios(
        self, 
        tool: ToolSearchResult, 
        unique_capabilities: List[str]
    ) -> List[str]:
        ...
        for capability in unique_capabilities:
            if capability == 'batch_editing':
                scenarios.append("When editing multiple files simultaneously")
            elif capability == 'pattern_matching':
                scenarios.append("When complex pattern matching is required")
            elif capability == 'workflow_control':
                scenarios.append("When workflow integration is important")
        
        # Based on tool name patterns
        if 'multi' in tool.name.lower():
            scenarios.append("When handling multiple items at once")
        elif 'advanced' in tool.name.lower():
            scenarios.append("When advanced features are needed")
    ...
    def _analyze_use_case_fit(
        ...
        elif 'pattern_matching' in alternative_capabilities and len(alternative_capabilities) > len(primary_capabilities):
    ...
    def _analyze_switching_cost(self, primary_tool: str, alternative_tool: str) -> Tuple[str, List[str]]:
        ...
        if similarity >= 0.8:
            cost = "low"
            considerations = [
                "Similar interface and parameters",
